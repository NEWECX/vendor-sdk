...
function validate_header(header, agreed_header, fields_maps, errors = []) {
    ...
    let use_fields_map = fields_maps;

    if (is_std_header(header, errors)) {
        use_fields_map = JSON.parse(JSON.stringify(use_fields_map));
        for (const field_map of use_fields_map) {
            delete field_map.field;
        }
        return ['std', use_fields_map];
    }

    if (agreed_header.length === 0) {
        push_errors(errors, 'header', 'error', 'agreed_header is empty');
        return [false, use_fields_map];    
    }
