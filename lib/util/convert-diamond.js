...
const { certificate_lab_map } = require('@ritani/diamond-glossary');
...
async function convert_diamond(row, fields_map, options, std_fields = standard_fields, asset_fields = Object.keys(assets_filename_map)) {
    ...
    if (diamond.pass && !diamond.cut && diamond.shape === 'RD') {
        diamond.pass = false;
        const field = 'cut';
    ...
    if (diamond.width && diamond.length && 
        ...
        if (diamond.depth && typeof diamond.depth === 'number') {
            ...
            diamond.depth_percent = Math.round(diamond.depth / (diamond.width + diamond.length) / 2 * 100);
        }
    }
        
    if (!certificate_lab_map[diamond.certificate_lab]) {
        if (diamond.pass) diamond.pass = false;
        diamond.certificate_lab = '';
    }

    if (!diamond.certificate_number) {
        if (diamond.pass) diamond.pass = false;
        diamond.certificate_number = '';
    }

    // fix certificate_number starts with lab
    if (diamond.certificate_lab && diamond.certificate_number && diamond.certificate_number.toUpperCase().startsWith(diamond.certificate_lab)) {
    ...
    if (diamond.certificate_lab && diamond.certificate_number) {
        if (['GIA', 'AGSL', 'GSI', 'HRD'].includes(diamond.certificate_lab)) {
            diamond.certificate_number = diamond.certificate_number.replace(/\D/g, '');
        } else if (diamond.certificate_lab === 'IGI') {
            diamond.certificate_number = diamond.certificate_number.toUpperCase();
            diamond.certificate_number = diamond.certificate_number.replace(/\W/g, '')
        } else if (diamond.certificate_lab === 'DF') {
            diamond.certificate_number = diamond.certificate_number.toUpperCase();
            diamond.certificate_number = diamond.certificate_number.replace(/[^0-9A-Z]/g, '')
        }
    }
