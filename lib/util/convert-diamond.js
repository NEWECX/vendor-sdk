 ...
 * 
 */
function convert_diamond(row, fields_map, additional_maps, std_fields = standard_fields, asset_fields = Object.keys(assets_filename_map)) {
    ...
    for (const item of std_fields) {
        ...
        if (value !== '') {
            ...
            if (values && !values.includes(value) && additional_maps && additional_maps[key]) {
                const uc_value = typeof value === 'string' ? value.toUpperCase() : uc_value;
                if (additional_maps[key][uc_value]) {
                    value = diamond[key] = additional_maps[key][uc_value];
                } else if (additional_maps[key].convert) {
                    const v = additional_maps[key].convert(uc_value);
                    if (v) {
                        value = diamond[key] = v;
                    }
                }
