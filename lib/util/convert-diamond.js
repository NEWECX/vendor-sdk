 ...
 * 
 */
function convert_diamond(row, fields_map, options, std_fields = standard_fields, asset_fields = Object.keys(assets_filename_map)) {
    ...
    const mapping_extension = options ? options.mapping_extension : null;
    ...
    for (const item of std_fields) {
        ...
        if (value !== '') {
            ...
            if (values && mapping_extension && !values.includes(value)) {
                try {
                    value = mapping_extension(key, value);
                } catch (err) {
                    console.error('ERROR: call mapping_extension', err);
                }
            }
