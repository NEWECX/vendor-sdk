...
function convert_diamond(row, fields_map, std_fields = standard_fields, asset_fields = Object.keys(assets_filename_map)) {
    ...
    if (diamond.pass) {
        ...
        } else if (Math.abs(diamond.carat * diamond.cost_per_carat - diamond.cost) >= 1) {
            diamond.pass = false;
            const calculated_cost = diamond.carat * diamond.cost_per_carat;
            push_errors(errors, 'cost', 'error', `cost (${diamond.cost.toFixed(2)}) not agrees with carat * cost_per_carat (${calculated_cost.toFixed(2)})`);
        }
    }
