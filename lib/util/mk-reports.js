...
module.exports = async (result, options) => {
    ...
    for (const diamond of diamonds) {
        ...
        if (!diamond.original_checksum) {
            diamond.original_checksum = get_checksum(diamond.original);
        }
...
async function summary_report(report_dir, header_ok, total, ok_count, diamonds_errors, diamonds_warnings, assets_stats) {
    ...
    assets_types_values.forEach(x => summary_fields.push('urls for ' + x));
    const summary_filepath = node_path.join(report_dir, 'summary.csv');
    const summary = [{'header ok': header_ok, 'total diamonds': total, 'passed count': ok_count, 'errors count': diamonds_errors.length, 'warnings count': diamonds_warnings.length}];
    for (const key in assets_stats) summary[0][key.replace(/_/g, ' ')] = assets_stats[key];
