...
function use_extra_mapping() {
    if (!config) {
        load_configuration();
    }
    if (config && config.use_extra_mapping) {
        return true;
    }
    return false;
}
...
function load_configuration() {
    if ((process.env.FTP_USER && process.env.FTP_PASSWORD) || process.env.API_KEY) {
        ...
        if (process.env.USE_EXTRA_MAPPING && (
            process.env.USE_EXTRA_MAPPING === '1' ||
            process.env.USE_EXTRA_MAPPING.toLowerCase() === 'true' ||
            process.env.USE_EXTRA_MAPPING.toLowerCase() === 'yes')) {
            config.use_extra_mapping = true;
        }
        if (!fs.existsSync(config.data_dir)) {
            fs.mkdirSync(config.data_dir, {recursive: true});
        }    
