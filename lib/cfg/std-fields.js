...
const std_fields = [
	...
	},
	{
		...
		require: 'required', 
		type: 'string',
		match: [{ in: ['cert', 'cert.*no']}, {nin: ['lab', 'type'] }]
	},
	{
	...
	},
	{
		...
		require: 'required', 
		transform: (x) => Math.round(x.carat * x.cost_per_carat * 100) / 100,
	...
	},
	{
		...
		require: 'required', 
		transform: (x) => x.carat ? Math.round(x.cost / x.carat * 100) / 100 : '',
		reducible: (x) => reducible(x, ['cost', 'carat']),
		type: 'number',
		match: [{in: ['cost', 'price', 'carat', '$']}, {in: ['per', '/']}, {nin: ['percent']}],
	},
	{
		key: 'clarity', 
	...
	},
	{
		...
		require: 'required',
		transform: (x) => { const p = x?.measurements?.split(/[ |*|\-|x|X]/).filter(x => x); if (p && p.length > 2) return p[2]; },
	...
	},
	{
		...
		require: 'required',
		transform: (x) => { const p = x?.measurements?.split(/[ |*|\-|x|X]/).filter(x => x); if (p && p.length > 1) { if (p[0] > p[1]) return p[1]; else return p[0]; }},
		reducible: (x) => reducible(x, ['measurements']),
		type: 'number',
	...
	},
	{
		...
		require: 'required', 
		transform: (x) => { const p = x?.measurements?.split(/[ |*|\-|x|X]/).filter(x => x); if (p && p.length > 1) { if (p[0] > [1]) return p[0]; else return p[1]; }},
		reducible: (x) => reducible(x, ['measurements']),
		type: 'number',
	...
	},
	{
		...
		transform: (x) => { const date = new Date(x.report_date); return !date || date === 'Invalid Date' ? '' : date.toISOString().substr(0, 10); },
		require: 'optional', 
		type: 'string',
