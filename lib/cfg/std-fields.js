...
const std_fields = [
	...
	},
	{
		...
		type: 'number',
		match: [{in: ['cost', 'price', 'per carat']}, {in: ['per', '/ct']}, {nin: ['percent']}],
	},
	{
		key: 'clarity', 
	...
	},
	{
		...
		require: 'required',
		transform: (x) => x?.measurements?.split(/[ |*|\-|x|X]/).filter(x => x).pop(),
	...
	},
	{
		...
		require: 'required',
		transform: (x) => { const p = x?.measurements?.split(/[ |*|\-|x|X]/).filter(x => x); if (p) { if (p[0] > p[1]) return p[1]; else return p[0]; }},
		reducible: (x) => reducible(x, ['measurements']),
		type: 'number',
	...
	},
	{
		...
		require: 'required', 
		transform: (x) => { const p = x?.measurements?.split(/[ |*|\-|x|X]/).filter(x => x); if (p) { if (p[0] > [1]) return p[0]; else return p[1]; }},
		reducible: (x) => reducible(x, ['measurements']),
		type: 'number',
	...
	},
	{
		...
		require: 'optional', 
		transform: (x) => (x.width > x.length ?  x.length : x.width) + ' - ' + (x.width > x.length ?  x.width : x.length) + ' x ' + x.depth,
		type: 'string',
		match: [{ in: ['measure']}]
	},
	{
