...
const std_fields = [
	...
	},
	{
		...
		//transform: (x) => Math.round(x.depth / (x.width + x.length) / 2 * 100), // delay to later util those values are calculated
		require: 'optional', 
		type: 'number',
	...
	},
	{
		...
		require: 'optional',
		transform: (x) => x.width ? Math.round(x.length / x.width * 100) / 100 : undefined,
		type: 'number',
