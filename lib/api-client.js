 ...
 * @returns api call response data
 */
async function api_get_inventory_info(ok_only = false, api_key = configuration.get_api_key()) {
 ...
 * @returns report
 */
async function api_get_report(reports_folder = os.tmpdir(), api_key = configuration.get_api_key()) {
 ...
 * @returns api call response data
 */
async function api_update_header(header, force = false, api_key = configuration.get_api_key()) {
 ...
 * @returns api call response data
 */
async function api_update_fields_maps(fields_maps, force = false, api_key = configuration.get_api_key()) {
 ...
 */
async function api_get_inventory(local_filepath, api_key = configuration.get_api_key()) {
    if (!api_key) {
    ...
    const result = await http_client(options);
    if (result && result.status === 200) {
        const stats = fs.statSync(local_filepath);
        if (stats.size === 0) {
            fs.rmSync(local_filepath);
            return {status: 'failed, empty file'};
        }
        return {status: 'OK', local_filepath};
    } else {
        return {status: 'failed'};
