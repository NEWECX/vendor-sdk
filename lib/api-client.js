 ...
 * @returns api call response data
 */
async function api_update_fields_maps(fields_map, api_key = configuration.get_api_key()) {

    if (!api_key) {
        throw new Error('update_fields_map, empty api_key');
    }
    if (!fields_map || !Array.isArray(fields_map)) {
        throw new Error('update_fields_map, fields_map must not empty and must be an array');
    }
    ...
    for (const item of fields_map) {
        for (const key in item) {
            if (!['key', 'field', 'values_map', 'default_value'].includes(key)) {
                delete item[key];
            }
        }
        if (!item.key) {
        ...
        if (item.values_map && typeof item.values_map === 'function') {
            const value = item.values_map.toString();
            sanitize_check(value);
            if (!value.startsWith('values_map(key, field, value, row) =>')) {
                throw new Error('incorrect values_map function pattern');
            }
            item.values_map = value;
        }
        new_fields_map.push(item);
    }
    ...
    const options = {method: 'post', url, data: new_fields_map, headers: {'content-type': 'application/json'}};
    const result = await http_client(options);
    if (result && result.data) {
        return result.data;
    } else {
