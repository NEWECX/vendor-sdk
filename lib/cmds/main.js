...
async function run_program(program, extension_options) {
    ...
    try {
        ...
        if (options.setConfig) {
            if (extension_options && extension_options.set_config) {
                await extension_options.set_config();
            } else {
                await set_config();
            }
        ...
        } else if (options.downloadInventory) {
        ...
        } else if (options.forceUploadHeader) {
            await upload_header(true);
        } else if (options.forceUploadFieldsMaps) {
            await upload_fields_maps(true);
        } else if (options.makeHeader) {
            await make_header(extension_options);
        }  else if (options.makeFieldsMaps) {
            await make_fields_maps(extension_options);
        } else if (options.validateInventory) {
            await validate_inventory(extension_options);
        } else if (options.retrieveAssets) {
            await retrieve_assets(extension_options);
        } else if (options.submitInventory) {
            await submit_inventory();
        } else if (options.submitStdInventory) {
            ...
            console.error('option not handled'.red, options);
            result = 1;
        }
