...
async function parse_inventory_csv(csv_filepath, fields_map, agreed_header, passed_callback) {
    if (!csv_filepath) {
    ...
    convertor.on('header', (header) => {
        ...
        // validate header
        //
        const result = validate_header(header, agreed_header, fields_map);
    ...
    convertor.subscribe((row, index) => {
        ...
        //
        const result = convert_diamond(row, fields_map);
        //console.log(result);
        const diamond = {row_no: index + 1, timestamp, ...result}
        if (diamond.pass) {
            if (passed_callback) passed_callback(diamond);
            ok_count++;
        }
    ...
    // prepare result
    //
    const result = {ok_count, total: diamonds.length, timestamp};
...
async function mk_inventory_reports(result) {
    ...
    summary_report(report_dir, total, ok_count, diamonds_errors, diamonds_warnings, assets_stats);
    errors_report(report_dir, diamonds_errors);
...
function summary_report(report_dir, total, ok_count, diamonds_errors, diamonds_warnings, assets_stats) {
