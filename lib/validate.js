...
async function parse_inventory_csv(csv_filepath, fields_map, agreed_header, options) {
    ...
    convertor.on('header', (header) => {
        ...
        // validate header
        //
        [header_ok, fields_map] = validate_header(header, agreed_header, fields_map, header_errors);
    ...
    convertor.subscribe((row, index) => {
        ...
        //
        const result = convert_diamond(row, fields_map, options);

        //console.log(result);

        const diamond = {row_no: index + 1, timestamp, ...result}

        if (options && options.item_parse_extension) {
            try {
                options.item_parse_extension(diamond, fields_map);
            } catch(err) {
                console.error('call item_parse_extension', err);
            }
        }
    ...
    if (options && options.items_extension) {
        try {
            if (!await options.items_extension(diamonds, header_errors)) {
                header_ok = false;
            }
        } catch(err) {
            console.error('call items_extension', err);
        }
    }
...
async function mk_inventory_reports(result) {
    ...
    std_inventory_report(report_dir, diamonds)
...
function std_inventory_report(report_dir, diamonds) {
