...
async function parse_inventory_csv(csv_filepath, fields_map, agreed_header, options) {
    ...
    convertor.subscribe(async (row, index) => {
        ...
        if (diamond.pass) {
            ...
            if (diamonds.find(x => x.certificate_lab === certificate_lab && x.certificate_number === certificate_number)) {
                diamond.pass = false;
                if (!diamond.errors) diamond.errors = [];
                push_errors(diamond.errors, 'certificate_number', 'error', `duplicated certificate ${certificate_lab}-${certificate_number}`);
            } else {
                ok_count++;
            }
        }
