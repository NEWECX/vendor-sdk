...
async function parse_inventory_csv(csv_filepath, fields_map, agreed_header, options) {
    ...
    convertor.subscribe((row, index) => {
        ...
        if (options && options.callback) {
            try {
                options.callback(diamond, fields_map);
            } catch(err) {
                console.error(err);
            }
        }
    ...
    if (options && options.diamonds_callback) {
        try {
            if (!await options.diamonds_callback(diamonds, header_errors)) {
                header_ok = false;
            }
        } catch(err) {
            console.error(err);
...
async function mk_inventory_reports(result) {
    ...
    console.log(`\nheader_ok: ${header_ok}`, `total diamonds: ${total}`, `passed count: ${ok_count}`);
    console.log();
