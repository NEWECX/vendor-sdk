...
async function parse_inventory_csv(csv_filepath, fields_map, agreed_header, options) {
    ...
    convertor.subscribe(async (row, index) => {
        ...
        if (diamond.pass) {
            ok_count += get_ok_count_inc_with_duplicate_checking(diamonds, diamond);
        }
...
function get_ok_count_inc_with_duplicate_checking(diamonds, diamond) {
    const {certificate_lab, certificate_number} = diamond;
    const existed_passed = diamonds.find(x => x.pass && x.certificate_lab === certificate_lab && x.certificate_number === certificate_number);
    if (!existed_passed) {
        return 1;
    }
...
async function mk_inventory_reports(result, options) {
    ...
    for (const diamond of diamonds) {
        ...
        const original = { ...diamond.original };
        for (const key in original) {
            if (/[$|.]/.test(key)) {
                const new_key = key.replace(/\$/g, '__dollar__').replace(/\./g, '__dot__');
                original[new_key] = original[key];
                delete original[key];
            }
        }
        diamond.checksum = get_checksum(original);
