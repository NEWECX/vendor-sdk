 ...
 */
 module.exports = async (row, fields_map, options, std_fields = standard_fields, asset_fields = Object.keys(assets_filename_map)) => {
    ...
    // pass 4 key_value_extension and validate
    //
    ...
    for (const item of std_fields) {
        ...
        if (key_value_extension && values && !values.includes(value)) {
            try {
                value = diamond[key] = await key_value_extension(key, value);
            } catch (err) {
                ...
                if (!value.startsWith('http://') && !value.startsWith('https://')) {
                    ...
                    try {
                        ...
                        if (!['http:', 'https:'].includes(protocol)) {
                            diamond[key] = '';
                            error_with_message(errors, diamond, field, key, value, require, 'protocol not http and https');
                            continue;
                        }
                    } catch(err) {
                        diamond[key] = '';
